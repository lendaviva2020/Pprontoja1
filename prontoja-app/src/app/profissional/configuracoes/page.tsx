...
import { User, Mail, Shield, Bell, Loader2, Save, CreditCard } from "lucide-react";
...
export default function ConfiguracoesProfissionalPage() {
  ...
  async function loadProfile() {
    ...
    const { data: profile } = await supabase
      .from("profiles")
      .select("full_name, display_name, phone_masked, metadata")
      .eq("id", user.id)
      .single();
    ...
    if (profile) {
      ...
      setHasStripe(!!(profile.metadata as Record<string, unknown>)?.stripe_account_id);
    }
    setLoading(false);
  }
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Gerencie sua conta profissional</p>
      </div>
      ...
      <div className="space-y-6">
        <div className="card p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="text" className="input-field" placeholder="Como quer ser chamado" value={displayName} onChange={e => setDisplayName(e.target.value)} />
            </div>
            <div>
        ...
        <div className="card p-6">
          <h2 className="flex items-center gap-2 text-lg font-semibold text-gray-900 mb-4">
            <CreditCard className="h-5 w-5 text-brand-600" /> Pagamentos
          </h2>
          <p className="text-sm text-gray-600 mb-3">
            {hasStripe ? "Sua conta Stripe Connect está configurada." : "Configure sua conta para receber pagamentos."}
          </p>
          <Link href="/profissional/stripe-connect" className="btn-secondary inline-flex items-center gap-2 text-sm">
            <CreditCard className="h-4 w-4" /> {hasStripe ? "Gerenciar Stripe" : "Configurar Stripe Connect"}
          </Link>
        </div>
